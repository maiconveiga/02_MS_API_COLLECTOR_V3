...
# # App + CORS
# # ---------------------------------------------------------
# app = FastAPI(title="API_COLLECTOR_V3", version="0.8.6")
...
#   # cria nova tabela temporária no formato V2
#   await db.execute("""
...
#     # Garante comments no formato V2 em bancos novos
...
# # ---------------------------------------------------------
# # Comments (V2 usando alarm_reference) — agora persistindo "Condição Normalizada"
# # com supressão de duplicados consecutivos (idempotência).
# # ---------------------------------------------------------
# class _CommentSQL:
...
#   async with aiosqlite.connect(DB_PATH) as db:
#     await db.execute("PRAGMA foreign_keys=ON;")
#     # verifica último comentário daquele reference
...
#         # idempotente: não cria; retorna o último existente
...
#   """
#   Remove alarmes que não possuem comentários associados.
#   Associação é feita por itemReference (alarms.itemReference) ↔ comments.alarm_reference.
#   """
#   async with aiosqlite.connect(DB_PATH) as db:
#     await db.execute("PRAGMA foreign_keys=ON;")
...
# App + CORS
# ---------------------------------------------------------
app = FastAPI(title="API_COLLECTOR_V3", version="0.9.0-perf")
...
def _build_client(verify_ssl: bool) -> httpx.AsyncClient:
  ...
  return httpx.AsyncClient(
    ...
    headers={
      ...
      "user-agent": "api-collector-v3/0.9.0",
    },
  )
